...
| `DAILY_SOURCE_PRIORITY` | Ordered, comma-separated lowercase provider names for A-share daily K-lines. Listed providers are tried first; omitted providers retain their numeric-priority order afterward. GitHub Actions defaults to `tencent,baostock,akshare,pytdx,yfinance,efinance,tushare,tickflow`. | empty | Optional |
| `EFINANCE_PRIORITY` | Numeric priority for Efinance daily K-lines. It now defaults to the degraded priority `6` instead of P0. An explicit `DAILY_SOURCE_PRIORITY` order takes precedence. | `6` | Optional |
| `TENCENT_PRIORITY` | Numeric priority for Tencent direct A-share daily K-lines; lower values are tried earlier. Defaults to `5`, while GitHub Actions promotes it through `DAILY_SOURCE_PRIORITY`. Does not affect realtime quotes. | `5` | Optional |
...
> Behavior notes:
> - GitHub Actions daily reports try Tencent, Baostock, AkShare, PyTDX, and YFinance before Efinance, Tushare, and TickFlow by default. A repository variable named `DAILY_SOURCE_PRIORITY` can override this order. Provider exceptions, empty results, and short-term circuit-open states emit structured `[数据源回退]` logs and continue to the next provider.
> - One daily-data API failure does not stop a report. The same stock falls back automatically; if every provider fails, only that stock continues with stored data or is marked failed, while other stock tasks and report aggregation continue.
